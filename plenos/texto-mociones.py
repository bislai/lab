...
day = input("Introduce el dia del pleno: ")

month = input("Introduce el mes del pleno: ")
...
with open('pleno-del-' + day + '-del-' + month + '.csv', 'w') as f:
    ...
    for elem in motionsLinks:
        # Nos quedamos con el contenido del enlace
        political = elem.contents[0]
        ...
        numberMotion = elem.previousSibling
        ...
        textMotion = elem.nextSibling.replace(',', ' ')

        # Ahora le pasamos todos los parametros al CSV que hemos creado anteriormente
        fileCSV.writerow([numberMotion, political, link, textMotion])
